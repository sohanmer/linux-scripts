...
echo "u: CPU details"
echo "d: Disk usage"
echo "n [option]: Network (available option:- ALL, DEV, ICMP, EDEV, FC, EICMP, ICMP6, EICMP6, IP, EIP, IP6, EIP6, NFS, NFSD, SOCK, SOCK6, SOFT, TCP, ETCP, UDP, UDP6)"
read report_option
...
do
	...
	do
		...
		then
			file_name="${server}_disk_usage.txt"
		elif [ "$j" == "A" ]
		then
		...
		then
			file_name="${server}_paging_stats.txt"
                ...
                then
                        file_name="${server}_network_stats.txt"
